...
{:else}
	{#each mutualGuilds as { name, icon, id, moderator }}
		{#if moderator || type !== undefined}
			<a href='/{type}{separator}{id}' class='guild' transition:slide>
				<span class='guildIcon'>
					{#if icon != null}
						<img src='https://cdn.discordapp.com/icons/{id}/{icon}' alt='Guild' />
					{/if}
				</span>
				<p>{name}</p>
			</a>
